...
response_size_counter_limit=100
...
do
	echo ">> Current BW: "$BW_counter" Mbps"
  	...
  	#sh simulate.sh  $i
	response_size_counter=1
	response_size_current=$response_size_initial
	platformFile="choreography_platform_failure"$BW_counter".xml"
	echo "platform: "$platformFile
	FileName="chor_data_failure_"$BW_counter"Mbps.dat"
	...
	do
		...
		sh simulate2.sh  $nro_request_current $response_size_current $FileName $platformFile
